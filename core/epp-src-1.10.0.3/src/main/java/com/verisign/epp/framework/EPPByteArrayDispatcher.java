...
public class EPPByteArrayDispatcher {
	...
	private static Logger LOG = Logger.getLogger(
			EPPByteArrayDispatcher.class.getName(),
	 ...
	 *                messages. <code>EPPAssemberException.isFatal</code> can be
	...
	public byte[] processMessage(byte[] aInputBytes, Object aData)
		...
		if (message instanceof EPPResponse) {
			LOG.info("processMessage(): Invalid message type "
					+ message.getClass().getName() + " for dispatching");
		...
		EPPEventHandler handler = (EPPEventHandler) this.eventHandlers
				.get(namespace);
		if (handler == null) {
			LOG.info("processMessage(): Handler not found for Namespace "
					+ namespace);
