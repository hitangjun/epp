...
public class EPPSignedMark implements EPPCodecComponent {
	...
	private Element parseAndGetDocElement ( byte[] aSignedMarkArray, boolean aIgnoreParsingError )
		...
		try {
			...
			cat.debug( "parseAndGetDocElement():Got SAXParseException while decoding signed mark", ex );
			...
			cat.debug( "parseAndGetDocElement():Got SAXException while decoding signed mark", ex );
			throw new EPPDecodeException( "[SAXException] " + ex, EPPCodecException.SAX_EXCEPTION );
		}
		catch ( Exception ex ) {
