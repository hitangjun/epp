...
/**
 * Factory for the Reseller Extension objects. The Reseller Extension only
 * supports command, response extensions with <code>createExtension</code>, but
 * currently does not support protocol extensions with
 * <code>createProtocolExtension</code>. Calling
 * <code>createProtocolExtension</code> will result in an exception.
 */
public class EPPResellerExtFactory extends EPPExtFactory {
	/** XML Namespace */
	public static final String NS = "urn:ietf:params:xml:ns:resellerext-1.0";

	/** XML Namespace Prefix */
	public static final String NS_PREFIX = "resellerext";

	/** EPP IDN Map Extension XML Schema. */
	public static final String NS_SCHEMA = "urn:ietf:params:xml:ns:resellerext-1.0 resellerext-1.0.xsd";

	/**
	 * Service object associated with EPPResellerExtFactory. The service object is
	 * used when creating the Greeting or the Login.
	 */
	private EPPService service;

	/**
	 * Default constructor for <code>EPPResellerExtFactory</code>.
	 */
	...
	public EPPCodecComponent createExtension(Element aExtensionElm)
		...
		if (name.equals(EPPUtil.getLocalName(EPPResellerExtInfData.ELM_NAME))) {
			return new EPPResellerExtInfData();
		}
		else if (name.equals(EPPUtil.getLocalName(EPPResellerExtCreate.ELM_NAME))) {
			return new EPPResellerExtCreate();
		}
		else if (name.equals(EPPUtil.getLocalName(EPPResellerExtUpdate.ELM_NAME))) {
			return new EPPResellerExtUpdate();
		}
		else {
	...
	public EPPProtocolExtension createProtocolExtension(Element aExtensionElm)
		...
		throw new EPPCodecException(
				"EPPResellerExtFactory.createProtocolExtension: Protocol extensions not supported");
	}
	...
	public Set getXmlSchemas() {
		...
		theSchemas.add("resellerext-1.0.xsd");
		return theSchemas;
	}
