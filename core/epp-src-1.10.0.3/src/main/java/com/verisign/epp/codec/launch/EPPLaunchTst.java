...
public class EPPLaunchTst extends TestCase {
	...
	public void testSignedMark() {
		 ...
		 */
		try {
			signedMark = new EPPSignedMark("2-2", issuer,
					new GregorianCalendar(2009, 8, 16).getTime(),
					new GregorianCalendar(2010, 8, 16).getTime(), mark);
		...
		try {
			...
			// int compareVal = (new String(signedMarkXml)).compareTo(new
			// String(
			// signedMarkXml2));
			// if (compareVal != 0) {
		...
		/**
		 * [3.1] Test creating immutable signed mark from a signed mark and
		 * validating it.
		 */
		...
		try {
			signedMark = new EPPSignedMark("5-2", issuer,
	...
	/**
	 * Tests the <code>EPPSignedMark</code> class using the exact attributes
	...
	public void testTrademarkHolderEnglishActive() {
		...
		trademark.setRegDate(EPPUtil
				.decodeTimeInstant("2012-12-31T23:00:00.000Z"));
		trademark.setExDate(EPPUtil
				.decodeTimeInstant("2014-12-31T23:00:00.000Z"));
		...
		try {
			signedMark = new EPPSignedMark("0000001701373633628125-65535",
					issuer,
		...
		try {
			...
			byte[] smdBase64 = Base64.encodeBase64(smdXml, true);
			FileOutputStream outStream = new FileOutputStream(
					"VRSN-Trademark-Holder-English-Active.smd");
			outStream.write("-----BEGIN ENCODED SMD-----\n".getBytes());
			outStream.write(smdBase64);
			outStream.write("-----END ENCODED SMD-----\n".getBytes());
			outStream.close();
		...
		try {
			FileOutputStream outStream = new FileOutputStream(
					"VRSN-Trademark-Holder-English-Active.xml");
			outStream.write(signedMark.toString().getBytes());
			outStream.close();
		}
		catch (Exception ex) {
	...
	/**
	 * Tests the <code>testLaunchClaimsCheckForm</code> check command extension. The tests
	 * include the following:<br>
	 * <br>
	 * <ol>
	 * <li>Test Claims Check Form Command during the "claims" phase
	 * <li>Test Claims Check Form Command during the "open" phase
	 * <li>Test Claims Check Form Response
	 * </ol>
	 */
	public void testLaunchClaimsCheckForm() {
		...
		// Claims Check Form Command during the "claims" phase
		...
		// Claims Check Form Command during the "open" phase
		theCommand = new EPPDomainCheckCmd("ABC-12345",
				domains);

		theCommand.addExtension(new EPPLaunchCheck(new EPPLaunchPhase(
				EPPLaunchPhase.PHASE_OPEN), EPPLaunchCheck.TYPE_CLAIMS));

		stats = EPPCodecTst.testEncodeDecode(theCommand);
		System.out.println(stats);
		
		
		// Claims Check Form Response during the "claims" phase
		...
		EPPLaunchCheckResult launchCheckResult = new EPPLaunchCheckResult(
				"example3.tld", true);
		launchCheckResult.addClaimKey(
				"2013041500/2/6/9/rJ1NrDO92vDsAzf7EQzgjX4R0000000001", "tmch");
		launchCheckResult.addClaimKey(
	...
	/**
	 * Tests the <code>testLaunchAvailabilityCheckForm</code> check command extension. The tests
	 * include the following:<br>
	 * <br>
	 * <ol>
	...
	/**
	 * Tests the <code>testLaunchTrademarkCheckForm</code> check command extension. The tests
	 * include the following:<br>
	 * <br>
	 * <ol>
	 * <li>Test Trademark Check Form Command
	 * <li>Test Trademark Check Form Response
	 * </ol>
	 */
	public void testLaunchTrademarkCheckForm() {
		...
		theCommand.addExtension(new EPPLaunchCheck(EPPLaunchCheck.TYPE_TRADEMARK));
	...
	public void testLaunchCreateCodeMark() {
		...
		createExt.addCodeMark(new EPPLaunchCodeMark("49FD46E6C4B45C55D4AE",
				"sample2"));
	...
	public void testLaunchCreateNotice() {
		...
		// Test create command for claims create using the notice information
		// and validatorID's
		EPPLaunchCreate launchCreate = new EPPLaunchCreate(new EPPLaunchPhase(
				EPPLaunchPhase.PHASE_CLAIMS));
		launchCreate.setType(EPPLaunchCreate.TYPE_REGISTRATION);
		launchCreate.addNotice(new EPPLaunchNotice(
				"370d0b7c9223372036854775807", new Date(), new Date(), "tmch"));
		launchCreate.addNotice(new EPPLaunchNotice("470d0b7c9223654313275808",
				new Date(), new Date(), "custom-tmch"));
