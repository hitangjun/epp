...
public class EPPSessionPoolableFactory implements PoolableObjectFactory {
	...
	/**
	 * Name or IP address to connect from. When <code>null</code> the host will
	 * be set to the loop back.
	 */
	...
	/**
	 * Sets the TCP server IP address or host name or the URL of the HTTP
	 * server.
	 * 
	 * @param aServerName
	...
	/**
	 * Gets the TCP server port number. This will be <code>null</code> if
	 ...
	 * <code>null</code> value will use the loop back.
	 * 
	 * @return Client host name or IP address if defined;<code>null</code>
	 *         otherwise.
	 */
	 ...
	 *            Client transaction identifier generator object or
	 *            <code>null</code> to unset the generator
	 */
	public void setClientTransIdGenerator(
	...
	public void destroyObject(Object aSession) throws Exception {
		...
		try {
			...
			if (this.clientTransIdGenerator != null) {
				theSession.setTransId(
						this.clientTransIdGenerator.genClientTransId());
			}
	...
	public Object makeObject() throws Exception {
		...
		if (this.clientTransIdGenerator != null) {
			theSession
					.setTransId(this.clientTransIdGenerator.genClientTransId());
		}
	...
	public boolean validateObject(Object aSession) {
		...
		try {
			...
			if (currentTime - thePooledSession.getCreatedTime() > this
					.getAbsoluteTimeout()) {
				log.debug("validateObject(): session id = " + aSession
						+ " is past absolute timeout");
				isValid = false;
			} // Idle timeout?
			else if (System.currentTimeMillis()
					- thePooledSession.getLastTouched() > this
							.getIdleTimeout()) {
				log.debug("validateObject(): session id = " + aSession
						+ " is past idle timeout, sending hello");
				theSession.hello();
	...
	protected EPPSession makeSession() throws Exception {
		...
		if (this.serverName == null || this.serverPort == null) {
			throw new EPPSessionPoolException(
					"makeSession(): serverName or serverPort is null");
		}

		if (this.clientHost == null) {
			return new EPPPooledGenericSession(this.serverName,
					this.serverPort.intValue(), this.sslContext);
		}
		else {
			return new EPPPooledGenericSession(this.serverName,
					this.serverPort.intValue(), this.clientHost,
					this.sslContext);
		}
