 ...
 * client by the server.</li>
 * <li>
 * A &ltpw&gt element that contains the client's plain text password. The value
 * of this element is case sensitive.</li>
 * <li>
 * An OPTIONAL &ltnewPW&gt element that contains a new plain text password to be
 * assigned to the client for use with subsequent &ltlogin&gt commands. The
 ...
 * <ul>
 * <li>
 * An &ltoptions&gt element that contains the following child elements:</li>
 * <li>
 * A &ltversion&gt element that contains the protocol version to be used for the
 * command or ongoing server session.</li>
 * <li>
 * A &ltlang&gt element that contains the text response language to be used for
 * the command or ongoing server session commands.</li>
 * </ul>
 * 
 * <br>
 * The values of the &ltversion&gt and &ltlang&gt elements MUST exactly match
 * one of the values presented in the EPP greeting. <br>
 * 
 * <ul>
 * <li>
 ...
 * element that contains one or more &ltextURI&gt elements that identify object
 * extensions to be used during the session.</li>
 * </ul>
 * 
 * <br>
...
public class EPPLoginCmd extends EPPCommand {
	...
	/** Class logger */
	private static final Logger cat = Logger.getLogger(EPPLoginCmd.class
	 ...
	 * <ul>
	 * <li>
	 * transaction id is set to <code>null</code>.</li>
	 * <li>
	 * client id is set to <code>null</code></li>
	 * <li>
	 * password is set to <code>null</code></li>
	 * <li>
	 * new password is set to <code>null</code></li>
	 * <li>
	 ...
	 * The other attributes are initialized as follows: <br>
	 * <br>
	 * 
	 * <ul>
	 * <li>
	...
	/**
	 * Allocates a new <code>EPPLoginCmd</code> with the required attributes and
	 * the optional new password attribute. The services is initialized based on
	 * the <code>EPPFactory</code> configuration.
	 *
	 * @param aTransId
	 *            transaction id of the command.
	 * @param aClientId
	 ...
	 * languages of the <code>EPPGreeting</code>. The default language is "en".
	 *
	 ...
	 *
	 * @param someServices
	 *            <code>Vector</code> of desired <code>EPPService</code>
	 *            instances
	 */
	 ...
	 * 
	 * @return <code>true</code> if the login services include the
	 *         service Namespace URI; <code>false</code> otherwise.
	 */
	public boolean hasService(String aNamespaceURI) {
		...
		if (this.services != null) {
			...
			while (!found && theSvcEnum.hasMoreElements()) {
				EPPService theExtService = (EPPService) theSvcEnum
						.nextElement();
	 ...
	 * 
	 * @return <code>true</code> if the login extension services include the
	 *         extension service Namespace URI; <code>false</code> otherwise.
	 */
	public boolean hasExtensionService(String aNamespaceURI) {
		...
		if (this.extservices != null) {
			...
			while (!found && theExtSvcEnum.hasMoreElements()) {
				EPPService theExtService = (EPPService) theExtSvcEnum
						.nextElement();
	 ...
	 * EPP Server will retrieve the list of extension services desired by the
	 * EPP Client.
	 *
	 ...
	 *            Vector of <code>EPPService</code> instances.
	 */
	...
	/**
	 * encode <code>EPPLoginCmd</code> into a DOM element tree. The &ltlogin&gt
	 ...
	 * <code>EPPCommand</code> provides the public <code>encode</code> and calls
	 * the abstract <code>doGenEncode</code>.
	 *
	 * @param aDocument
	 *            DOCUMENT ME!
	 *
	...
	protected Element doGenEncode(Document aDocument) throws EPPEncodeException {
		...
		// Client Id
		EPPUtil.encodeString(aDocument, root, clientId, EPPCodec.NS,
				ELM_CLIENT_ID);

		// Password
		EPPUtil.encodeString(aDocument, root, password, EPPCodec.NS,
				ELM_PASSWORD);

		// New Password
		EPPUtil.encodeString(aDocument, root, newPassword, EPPCodec.NS,
				ELM_NEW_PASSWORD);

		// Options
		Element optionsElm = aDocument
				.createElementNS(EPPCodec.NS, ELM_OPTIONS);
		root.appendChild(optionsElm);

		// Version
		EPPUtil.encodeString(aDocument, optionsElm, version, EPPCodec.NS,
				ELM_VERSION);
		...
		Element servicesElm = aDocument.createElementNS(EPPCodec.NS,
				ELM_SERVICES);
		...
		if ((extservices != null) && extservices.elements().hasMoreElements()) {
			...
			Element svcExtension = aDocument.createElementNS(EPPCodec.NS,
					ELM_EXT_SERVICES);
	 ...
	 * element needs to be the value of the <code>aElement</code> argument. This
	 * method is part of the Template Design Pattern, where
	 * <code>EPPCommand</code> provides the public <code>decode</code> and calls
	 * the abstract <code>doGenDecode</code>.
	 *
	 * @param aElement
	 *            &ltlogin&gt root element tree.
	 *
	...
	protected void doGenDecode(Element aElement) throws EPPDecodeException {
		...
		// New Password
		newPassword = EPPUtil.decodeString(aElement, EPPCodec.NS,
				ELM_NEW_PASSWORD);

		// Options
		NodeList elms = aElement.getElementsByTagNameNS(EPPCodec.NS,
				ELM_OPTIONS);
		...
		if (elms.getLength() == 1) {
			...
			// Version
			version = EPPUtil
					.decodeString(optionsElm, EPPCodec.NS, ELM_VERSION);
		...
		if (servicesElm == null) {
			throw new EPPDecodeException(
					"EPPLoginCmd.doGenDecode did not find a " + ELM_SERVICES
							+ " element");
		}
	 ...
	 *
	 * @param aObject
	 *            <code>EPPLoginCmd</code> instance to compare with
	 *
	...
	public boolean equals(Object aObject) {
		...
		// clientId
		if (!((clientId == null) ? (theLogin.clientId == null) : clientId
				.equals(theLogin.clientId))) {
			return false;
		}

		// password
		if (!((password == null) ? (theLogin.password == null) : password
				.equals(theLogin.password))) {
			return false;
		}

		// newPassword
		if (!((newPassword == null) ? (theLogin.newPassword == null)
				: newPassword.equals(theLogin.newPassword))) {
			return false;
		}

		// version
		if (!((version == null) ? (theLogin.version == null) : version
				.equals(theLogin.version))) {
			return false;
		}

		// lang
		if (!((lang == null) ? (theLogin.lang == null) : lang
				.equals(theLogin.lang))) {
			return false;
		}
	...
	public Object clone() throws CloneNotSupportedException {
		...
		for (int i = 0; i < services.size(); i++)
			clone.services.setElementAt(
					((EPPService) services.elementAt(i)).clone(), i);
	 ...
	 * the services specified in the <code>EPPGreeting</code>? The services
	 * attributes defined in <code>EPPLoginCmd</code> must be a subset of the
	 * available services specified in the <code>EPPGreeting</code>.
	 *
	 * @param aGreeting
	...
	public boolean isValidServices(EPPGreeting aGreeting) {
		...
		// Services
		if (!EPPUtil.vectorSubset(services,
	...
	public void mergeServicesAndExtensionServices(EPPGreeting aGreeting) {
		...
		this.extservices.retainAll(aGreeting.getServiceMenu()
