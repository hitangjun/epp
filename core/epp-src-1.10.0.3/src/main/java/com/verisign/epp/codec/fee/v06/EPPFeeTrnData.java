...
public class EPPFeeTrnData extends EPPFeeTransformResult {
	...
	protected void validateAttributes() throws EPPEncodeException {
		...
		if (!super.hasFees()) {
			throw new EPPEncodeException(
					"At least one fee is not set for EPPFeeTrnData on call to encode(Document).");
		}

		// Invalid base class attributes
		if (super.hasCredits()) {
			throw new EPPEncodeException(
					"At least one fee is incorrectly set for EPPFeeTrnData on call to encode(Document).");
		}
	}
	...
	public EPPFeeTrnData(String aCurrency, EPPFeePeriod aPeriod,
			List<EPPFeeValue> aFees, BigDecimal aBalance,
