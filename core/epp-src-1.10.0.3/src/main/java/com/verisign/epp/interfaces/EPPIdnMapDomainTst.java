...
/**
 * Is a unit test of the <code>EPPDomain</code> class with the IDN Map
 * Extension. The unit test will initialize a session with an EPP Server, will
 * invoke <code>EPPDomain</code> operations with the IDN Map Extension, and will
 ...
 */
public class EPPIdnMapDomainTst extends TestCase {
	/**
	...
	/** Logging category */
	private static final Logger cat = Logger.getLogger(EPPIdnMapDomainTst.class
	 ...
	 */
	public EPPIdnMapDomainTst(String name) {
		super(name);
	}

	// End EPPIdnMapDomainTst(String)
	...
	public void domainInfo() {
		...
		try {
			System.out
					.println("\ndomainInfo: Domain info for xn--espaol-zwa.example.com to get the info response with the <code>EPPIdnMap</code> extension.");

			domain.setTransId("ABC-12345");

			domain.addDomainName("xn--espaol-zwa.example.com");
			...
			if (response.hasExtension(EPPIdnMap.class)) {
				EPPIdnMap theExt = (EPPIdnMap) response
						.getExtension(EPPIdnMap.class);
				System.out.println("domain:info: Response IDN table = "
						+ theExt.getTable());
				if (theExt.hasUname()) {
					...
					System.out
							.println("domain:info: Response IDN uname is undefined");
				}
			}
			else {
				System.out
	...
	public void domainCreate() {
		...
		try {
			...
			System.out
					.println("domainCreate: Create domain with EPPIdnMap extension");
			...
			// Add extension
			domain.addExtension(new EPPIdnMap("es", "spa\u00F1ol.example.com"));
	...
	public static Test suite() {
		TestSuite suite = new TestSuite(EPPIdnMapDomainTst.class);
	...
	public static void main(String[] args) {
		...
		if (numThreads > 1) {
			...
			junit.textui.TestRunner.run(EPPIdnMapDomainTst.suite());
		}
