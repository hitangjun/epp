...
public class EPPSessionTst extends TestCase {
	...
	private void initSession() {
		...
		String theClientId = Environment.getProperty("EPP.Test.clientId",
				"ClientX");
		String thePassword = Environment.getProperty("EPP.Test.password",
				"foo-BAR2");
	...
	/**
	 * Unit test of <code>EPPSession.sendPacket</code> command. This test will
	 * encode a Hello packet and send it using the
	 * {@link EPPSession#sendPacket(byte[])} method. The greeting will be read
	 * using the {@link EPPSession#readResponse()} method.
	 */
	private void doSendPacket() {
		printStart("doSendPacket");
		...
		try {
			...
			theDoc = session.recDocument();

			// Decode DOM Document to <code>EPPGreeting</code> instance
			try {
				EPPGreeting greeting = (EPPGreeting) EPPCodec.getInstance()
						.decode(theDoc);
				System.out.println(
						"doSendPacket: Greeting = [" + greeting + "]\n\n");
			}
			catch (EPPDecodeException ex) {
				Assert.fail(
						"doSendPacket(); Error reading the greeting after a hello");
			}
		}
		catch (EPPCommandException e) {
			handleException(e);
		...
		printEnd("doSendPacket");
	}

	/**
	 * Unit test of attempting to send the server an invalid request, which is
	 * sending the server an <code>EPPResponse</code> or an
	 * <code>EPPGreeting</code>>.
	 */
	private void doSendInvalidRequest() {
		printStart("doSendInvalidRequest");

		try {
			// Send an EPP Response
			...
			try {
				EPPResponse response = (EPPResponse) EPPCodec.getInstance()
						.decode(theDoc);
				System.out.println("doSendInvalidRequest: Response = ["
						+ response + "]\n\n");
				if (response.isSuccess()) {
					Assert.fail(
							"doSendInvalidRequest(); Error receiving successful response after sending invalid request (response)");
				}
			}
			catch (EPPDecodeException ex) {
				Assert.fail(
						"doSendInvalidRequest(); Error reading the response after sending invalid request (response)");
			}

			// Send an EPP Greeting
			EPPGreeting greeting = new EPPGreeting();
			greeting.setServer("doSendInvalidRequest");
			
			
			// Attempt to send an EPPGreeting
			...
			try {
				...
				if (response.isSuccess()) {
					Assert.fail(
							"doSendInvalidRequest(); Error receiving successful response after sending invalid request (greeting)");
				}
			}
			catch (EPPDecodeException ex) {
				Assert.fail(
						"doSendInvalidRequest(); Error reading the response after sending invalid request (greeting)");
			}
			...
			Assert.fail("doSendInvalidRequest(); General exception: " + e);
		}

		printEnd("doSendInvalidRequest");
	}
	...
	private void doPoll() {
		...
		try {
			...
			// Expect error
			System.out.println(
					"doPoll: Error Response = [" + e.getResponse() + "]\n\n");
		}
	...
	/**
	 * Unit test processing poll responses asynchronous from the commands. This
	 * is a test of the use of pipelining.
	 */
	private void doAsyncPoll() {
		...
		if (!session.isModeSupported(EPPSession.MODE_ASYNC)) {
			System.out.println(
					"doAsyncPoll: Session " + session.getClass().getName()
		...
		try {
			...
			Assert.assertNull("doAsyncPoll: sendPoll() response " + thePollResp
					+ " != null", thePollResp);
			...
			Assert.assertEquals(
					"doAsyncPoll: clientTransId's don't Match (command and response)",
					theClientTransId,
		...
		try {
			...
			if (theSessionClassName != null) {
				try {
					...
					Assert.fail(
							"Exception instantiating EPP.SessionClass value "
