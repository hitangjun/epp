...
public class EPPFeeCreData extends EPPFeeTransformResult {
	...
	protected void validateAttributes() throws EPPEncodeException {
		...
		if (!super.hasFees()) {
			throw new EPPEncodeException(
					"At least one fee is not set for EPPFeeCreData on call to encode(Document).");
		}

		// Invalid base class attributes
		if (super.hasCredits()) {
			throw new EPPEncodeException(
					"At least one fee is incorrectly set for EPPFeeCreData on call to encode(Document).");
		}
		if (super.hasPeriod()) {
			throw new EPPEncodeException(
