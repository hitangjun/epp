...
public class NSHostTst extends TestCase {
	...
	private static final Logger cat = Logger
			.getLogger(com.verisign.epp.namestore.interfaces.NSHostTst.class.getName(), EPPCatFactory.getInstance()
					.getFactory());
	...
	/**
	 * Allocates an <code>NSHostTst</code> with a logical name. The constructor
	...
	public static Test suite() {
		return new NSHostTstSetup(new TestSuite(com.verisign.epp.namestore.interfaces.NSHostTst.class));
	}
		...
		if (numThreads > 1) {
			...
			for (int i = 0; i < numThreads; i++) {
				TestThread thread = new TestThread("NSHostTst Thread " + i,
						com.verisign.epp.namestore.interfaces.NSHostTst.suite());
				thread.start();
			}
		}
		else { // Single threaded mode.
			junit.textui.TestRunner.run(com.verisign.epp.namestore.interfaces.NSHostTst.suite());
		}
	 ...
	 *
	 * @param newDomainName
	 *            Domain name to based host name on
	 *
